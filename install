...
main() {
    ...
    # Dừng và tắt systemd-resolved
    sudo systemctl stop systemd-resolved
    sudo systemctl disable systemd-resolved
    # Khởi động lại NextDNS
    nextdns restart
    ...
    while true; do
        ...
        if [ "$CURRENT_RELEASE" ]; then
            if ! is_version_current; then
                ...
                menu \
                    4 "Fix ERROR" configurefix \
                    3 "Upgrade NextDNS from $CURRENT_RELEASE to $INSTALL_RELEASE" upgrade \
                    2 "Configure NextDNS" configure \
                    1 "Remove NextDNS" uninstall \
                    0 "Exit" exit
            else
                log_debug "NextDNS is up to date ($CURRENT_RELEASE)"
                menu \
                    3 "Fix ERROR" configurefix \
                    2 "Configure NextDNS" configure \
                    1 "Remove NextDNS" uninstall \
                    0 "Exit" exit
            fi
            ...
            menu \
                2 "Fix ERROR" configurefix \
                1 "Install NextDNS" install \
                0 "Exit" exit
        fi
    done
}
